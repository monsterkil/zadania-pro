...
async function sendEmail({ to, subject, html, text }) {
  ...
  const hasSmtp = smtpAccount && String(smtpAccount).trim();
  if (!hasSmtp) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      return { success: false, reason: "api_error", data, message: apiMessage(data) };
    }
    console.error("[EmailLabs] Błąd HTTP", res.status, data);
    return { success: false, reason: "api_error", data, message: apiMessage(data) || `HTTP ${res.status}` };
  } catch (err) {
