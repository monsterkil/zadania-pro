...
async function sendEmail({ to, subject, html, text }) {
  const appKey = (process.env.EMAILLABS_APP_KEY || "").trim();
  const secretKey = (process.env.EMAILLABS_SECRET_KEY || "").trim();
  const smtpAccount = (process.env.EMAILLABS_SMTP_ACCOUNT || "").trim();
  const fromEmail = (process.env.EMAIL_FROM || "zadania@biostima.pl").trim().toLowerCase();
  const fromName = (process.env.EMAIL_FROM_NAME || "Zadania Pro").trim();
  ...
  if (!smtpAccount) {
    console.warn("[EmailLabs] Brak EMAILLABS_SMTP_ACCOUNT — ustaw w env (np. 1.biostima.smtp)");
    return { success: false, reason: "no_smtp_account" };
  }
  ...
  try {
    ...
    const apiMessage = (d) => {
      ...
      const s = d.message || d.error || d.msg || d.err || (Array.isArray(d.errors) && d.errors[0]?.message) || (d.errors && d.errors[0]);
      if (typeof s === "string") return s.slice(0, 200);
      if (s && typeof s === "object" && s.message) return String(s.message).slice(0, 200);
      try {
    ...
    if (res.ok) {
      ...
      const msg1 = apiMessage(data);
      if (msg1 && /sender|nadawc|from.*valid/i.test(msg1)) {
        console.error("[EmailLabs] Odrzucono nadawcę. Wysłano: from=" + fromEmail + " smtp_account=" + smtpAccount + " — w Vercel sprawdź EMAIL_FROM i EMAILLABS_SMTP_ACCOUNT (bez spacji); w panelu — czy domena jest przy tym kontie.");
      }
      return { success: false, reason: "api_error", data, message: msg1 };
    }
    console.error("[EmailLabs] Błąd HTTP", res.status, data);
    const msg2 = apiMessage(data) || `HTTP ${res.status}`;
    if (msg2 && /sender|nadawc|from.*valid/i.test(msg2)) {
      console.error("[EmailLabs] Odrzucono nadawcę. Wysłano: from=" + fromEmail + " smtp_account=" + smtpAccount);
    }
    return { success: false, reason: "api_error", data, message: msg2 };
  } catch (err) {
    console.error("[EmailLabs] Exception:", err.message);
    return { success: false, reason: "network_error", error: err.message, message: err.message };
  }
}
...
async function notifyNewTask(task) {
  const recipients = (process.env.EMAIL_NEW_TASK_TO || "damian@biostima.pl,sklep@biostima.pl").split(",").map((e) => e.trim()).filter(Boolean);
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "";

  const result = await sendEmail({
    to: recipients.length ? recipients : ["damian@biostima.pl"],
