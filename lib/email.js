...
async function sendEmail({ to, subject, html, text }) {
  ...
  const fromEmail = process.env.EMAIL_FROM || "sklep@biostima.pl";
  ...
  // EmailLabs requires application/x-www-form-urlencoded
  // Recipients format: to[email@addr.com][message_id]=unique_id
  ...
  params.append("text", text || subject);

  recipients.forEach((email) => {
    const msgId = `zp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    params.append(`to[${email}][message_id]`, msgId);
  });
  ...
  try {
    ...
    if (res.ok && data.status !== "fail") {
      ...
      console.error("[EmailLabs] Błąd:", JSON.stringify(data));
